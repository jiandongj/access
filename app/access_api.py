...
class AccessApi:
    ...
    async def landing(self, request):
        ...
        return dict(
            agents=[
                a.display for a in await self.data_svc.locate("agents", match=search)
            ],
    ...
    async def exploit(self, request):
        ...
        converted_facts = [
            Fact(trait=f["trait"], value=f["value"]) for f in data.get("facts", [])
        ]
        await self.rest_svc.task_agent_with_ability(
            data["paw"], data["ability_id"], data["obfuscator"], converted_facts
        )
    ...
    async def get_abilities(self, request):
        ...
        return web.json_response([a.display for a in abilities])

    async def abilities(self, request):
        data = await request.json()
        agent_search = dict(
            access=tuple(await self.auth_svc.get_permissions(request)), paw=data["paw"]
        )
